...
namespace Visyde
{
    ...
    public class AIController : MonoBehaviour
    {
        ...
        {
            if (!player.TryGetComponent<Rigidbody2D>(out var rg)) return;
            ...
            else
            {
                ...
                if (!leftPlatform)
                {
                    leftPlatform = player.rg.velocity.y <= 0 && !SideHasGround(Mathf.RoundToInt(doMoveDir) * -1) && transform.position.y >= (lastFoundPlatformHeight.HasValue ? lastFoundPlatformHeight.Value : transform.position.y);
                }
